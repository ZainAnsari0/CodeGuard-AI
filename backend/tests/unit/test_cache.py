...
class TestCacheService:
    ...
    async def test_cache_get_returns_none_when_disabled(self):
        ...
        cache = PromptCache(enabled=False)
        result = await cache.get("vulnerability_analysis", "test prompt", "gpt-4")
        assert result is None

    @pytest.mark.asyncio
    async def test_cache_set_returns_false_when_disabled(self):
        """When cache is disabled, set should return False."""
        cache = PromptCache(enabled=False)
        result = await cache.set("vulnerability_analysis", "test prompt", "gpt-4", {"data": "test"})
        assert result is False

    @pytest.mark.asyncio
    async def test_cache_invalidate_returns_false_when_disabled(self):
        """When cache is disabled, invalidate should return False."""
        cache = PromptCache(enabled=False)
        result = await cache.invalidate("vulnerability_analysis")
        assert result is False
    ...
    async def test_cache_get_with_mock_redis(self):
        ...
        # Use MagicMock (not AsyncMock) because asyncio.to_thread runs sync functions
    ...
    @pytest.mark.asyncio
    async def test_cache_get_handles_redis_error(self):
